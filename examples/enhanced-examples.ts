...
import type { DatabaseSchema } from '../orm';

// Расширенная схема с валидацией
const enhancedSchema: DatabaseSchema = {
	users: {
		...
		constraints: [
			{
				...
				expression:
					'string::matches($value.username, "^[a-zA-Z0-9_]+$")',
			},
		],
		permissions: {
		...
		triggers: [
			{
				...
				event: 'UPDATE',
				expression:
					'IF $value.published = true AND $before.published = false THEN $value.publishedAt = time::now() END',
			},
		],
	},
...
export async function basicUsageExample() {
	...
	try {
		...
		try {
			...
			console.log('❌ Валидация сработала:', (error as Error).message);
		}
	} catch (error) {
...
export async function monitoringExample() {
	...
	metrics.on('threshold', (metric, threshold) => {
		console.warn(
			`⚠️ Порог превышен: ${metric.name} = ${metric.value} (порог: ${threshold})`
		);
	});
...
export async function fullExample() {
	...
	try {
		...
		const post = await postsTable.createRecord({
			...
			content:
				'This is a comprehensive guide to using the enhanced SurrealDB ORM with TypeScript...',
			author: user.id,
			published: true,
		...
		console.log(
			`📝 Найдено ${publishedPosts.length} опубликованных постов`
		);
