...
```typescript
import { DatabaseSchema } from "./orm.js";
...
indexes: [
	...
	},
	{
		name: "search_body",
		fields: ["body"],
		search: { analyzer: "english", bm25: { k1: 1.2, b: 0.75 }, highlights: true },
	},
];
```
...
```ts
const rows = await table.find("published = $pub AND string::len(title) > 3", { pub: true });
```
...
```ts
// Создать ребро
await orm.relate("wrote", "user:alice", "post:1", { at: new Date().toISOString() });
// Удалить ребро
