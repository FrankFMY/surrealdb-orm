...
import { MemoryCache, CacheManager, type CacheConfig } from '../../src/cache/index.js';
...
describe('CacheManager', () => {
  ...
  describe('schema caching', () => {
    it('should cache and retrieve schemas', async () => {
      ...
      expect(mockCache.set).toHaveBeenCalledWith('test_prefix:schema:users', schema, undefined);
      expect(mockCache.get).toHaveBeenCalledWith('test_prefix:schema:users');
    });
  });
  ...
  describe('query result caching', () => {
    it('should cache and retrieve query results', async () => {
      ...
      expect(mockCache.set).toHaveBeenCalledWith('test_prefix:query:query_hash', result, undefined);
      expect(mockCache.get).toHaveBeenCalledWith('test_prefix:query:query_hash');
    });
  });
  ...
  describe('metadata caching', () => {
    it('should cache and retrieve metadata', async () => {
      ...
      expect(mockCache.set).toHaveBeenCalledWith('test_prefix:metadata:version', metadata, undefined);
      expect(mockCache.get).toHaveBeenCalledWith('test_prefix:metadata:version');
    });
  });
  ...
  describe('cache invalidation', () => {
    it('should invalidate table cache', async () => {
      ...
      expect(mockCache.delete).toHaveBeenCalledWith('test_prefix:query:users_find');
      expect(mockCache.delete).toHaveBeenCalledWith('test_prefix:schema:users');
      expect(mockCache.delete).not.toHaveBeenCalledWith('test_prefix:query:posts_find');
    });

    it('should invalidate specific query cache', async () => {
      await cacheManager.invalidateQuery('query_hash');
      
      expect(mockCache.delete).toHaveBeenCalledWith('test_prefix:query:query_hash');
    });
