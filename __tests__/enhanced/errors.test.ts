...
describe('Error System', () => {
	describe('SurrealORMError', () => {
		it('should create error with message and context', () => {
			class TestError extends SurrealORMError {
				readonly code = 'TEST_ERROR';
				readonly statusCode = 400;
			}

			const error = new TestError('Test error', { table: 'users' });
	...
	describe('ErrorHandler', () => {
		...
		it('should detect error types by message', () => {
			...
			const normalizedConnection =
				ErrorHandler.normalizeError(connectionError);
		...
		it('should create user-friendly messages', () => {
			...
			expect(ErrorHandler.createUserMessage(validationError)).toBe(
				'Данные не прошли валидацию. Проверьте введенные значения.'
			);
			expect(ErrorHandler.createUserMessage(connectionError)).toBe(
				'Ошибка подключения к базе данных. Попробуйте позже.'
			);
			expect(ErrorHandler.createUserMessage(authError)).toBe(
