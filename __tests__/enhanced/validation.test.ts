...
describe('SchemaValidator', () => {
	...
	describe('validateSchema', () => {
		...
		it('should detect missing fields in table', () => {
			...
			expect(result.errors.some((e) => e.code === 'EMPTY_FIELDS')).toBe(
				true
			);
		});
		...
		it('should detect invalid index fields', () => {
			...
			expect(result.isValid).toBe(false);
			expect(
				result.errors.some((e) => e.code === 'INVALID_INDEX_FIELD')
			).toBe(true);
		});
	});
	...
	describe('validateRecord', () => {
		...
		it('should detect validation errors', () => {
			...
			expect(result.errors.some((e) => e.code === 'INVALID_EMAIL')).toBe(
				true
			);
			expect(
				result.errors.some((e) => e.code === 'MIN_VALUE_VIOLATION')
			).toBe(true);
			expect(
				result.errors.some((e) => e.code === 'UNIQUE_ITEMS_VIOLATION')
			).toBe(true);
		});
		...
		it('should handle unknown fields when allowUnknownFields is false', () => {
			...
			expect(result.isValid).toBe(false);
			expect(result.errors.some((e) => e.code === 'UNKNOWN_FIELD')).toBe(
				true
			);
		});
	});
	...
	describe('field validation', () => {
		it('should validate string constraints', () => {
			const data = {
				email: 'a@b', // too short (3 chars, minLength is 5)
			};
			...
			expect(result.isValid).toBe(false);
			expect(
				result.errors.some((e) => e.code === 'MIN_LENGTH_VIOLATION')
			).toBe(true);
		});
		...
		it('should validate number constraints', () => {
			...
			expect(result.isValid).toBe(false);
			expect(
				result.errors.some((e) => e.code === 'MAX_VALUE_VIOLATION')
			).toBe(true);
		});
		...
		it('should validate array constraints', () => {
			...
			expect(result.isValid).toBe(false);
			expect(
				result.errors.some((e) => e.code === 'MAX_ITEMS_VIOLATION')
			).toBe(true);
		});
	});
});
