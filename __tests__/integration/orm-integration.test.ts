...
} from '../../src/orm-enhanced';
import { SimpleLogger, SimpleFuture } from '../../helpers';
import type { StrictDatabaseSchema } from '../../src/types/validation';

// Проверка доступности SurrealDB сервера
async function isSurrealDBAvailable(): Promise<boolean> {
	try {
		const testConnection = new ConnectionManager(
			{
...
describe('Enhanced ORM Integration Tests', () => {
	...
	// Пропускаем все тесты, если сервер недоступен
	beforeAll(async () => {
		isServerAvailable = await isSurrealDBAvailable();
		if (!isServerAvailable) {
			console.log(
				'⚠️  SurrealDB server not available, skipping all integration tests'
			);
		}
	});

	beforeEach(async () => {
		// Пропускаем инициализацию, если сервер недоступен
		if (!isServerAvailable) {
			return;
		}
	...
	describe('CRUD Operations', () => {
		it('should create, read, update, and delete records', async () => {
			if (!isServerAvailable) {
				console.log('⚠️  Skipping test - SurrealDB not available');
				return;
			}
	...
	describe('Schema Validation', () => {
		...
		it('should reject invalid schema in strict mode', async () => {
			...
			const invalidOrm = createEnhancedORM(
				connection,
				queryEngine,
				invalidSchema,
				{
