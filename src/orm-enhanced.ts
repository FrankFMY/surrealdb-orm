...
import type { DatabaseSchema, TableConfig, FieldConfig } from './orm.js';
import type { StrictDatabaseSchema, SchemaValidator, ValidationContext } from './types/validation.js';
...
import { ValidationError, SchemaError, QueryError } from './errors/index.js';
import { SQLInjectionValidator, RateLimiter, AuditLogger } from './security/index.js';
...
export class EnhancedTable<Config extends TableConfig> {
  ...
  constructor(
    ...
    this.validator = new SchemaValidator({ [tableName]: config } as StrictDatabaseSchema);
  ...
  async createRecord(
    ...
    for (const [key, value] of Object.entries(data)) {
      if (typeof value === 'string' && !SQLInjectionValidator.validate(value)) {
        throw new QueryError(`Potential SQL injection detected in field: ${key}`);
      }
    }
  ...
  async find(
    ...
    if (!SQLInjectionValidator.validate(where)) {
      throw new QueryError('Potential SQL injection detected in WHERE clause');
    }

    // Проверка SQL инъекций в переменных
    for (const [key, value] of Object.entries(vars)) {
      if (typeof value === 'string' && !SQLInjectionValidator.validate(value)) {
        throw new QueryError(`Potential SQL injection detected in variable: ${key}`);
      }
    }
  ...
  getStats() {
    return {
      ...
      rateLimiter: this.rateLimiter ? {
        remainingRequests: this.rateLimiter.getRemainingRequests('read')
      } : null,
...
export class EnhancedSurrealORM<Schema extends DatabaseSchema> {
  private tables = new Map<keyof Schema, EnhancedTable<Schema[keyof Schema]>>();
  private validator: SchemaValidator;
  private cache?: CacheManager;
  private auditLogger?: AuditLogger;
   ...
   * Создание таблицы
   */
  private async createTable(tableName: string, table: EnhancedTable<any>): Promise<void> {
   ...
   */
  private generateCreateTableSQL(tableName: string, table: EnhancedTable<any>): string {
