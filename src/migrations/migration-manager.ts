...
export class MigrationManager {
	...
	registerMigration(migration: Migration): void {
		if (this.migrations.has(migration.version)) {
			throw new MigrationError(
				`Migration ${migration.version} already registered`
			);
		}

		this.migrations.set(migration.version, migration);
		this.logger.info(
			{ module: 'MigrationManager', method: 'registerMigration' },
	...
	async initialize(): Promise<void> {
      ...
      `);
			...
			this.logger.info(
				{ module: 'MigrationManager', method: 'initialize' },
	...
	private async loadAppliedMigrations(): Promise<void> {
      ...
      `);
			...
			this.logger.debug(
				{ module: 'MigrationManager', method: 'loadAppliedMigrations' },
				`Loaded ${this.appliedMigrations.size} applied migrations`
			);
		} catch (error) {
			this.logger.warn(
				{ module: 'MigrationManager', method: 'loadAppliedMigrations' },
				`Failed to load applied migrations: ${error instanceof Error ? error.message : 'Unknown error'}`
			);
		}
	}
	...
	createPlan(targetVersion?: string): MigrationPlan {
		...
		if (targetVersion) {
			...
			if (!targetMigration) {
				throw new MigrationError(
					`Target migration ${targetVersion} not found`
				);
			}
		...
		for (const migration of toApply) {
			if (migration.dependencies) {
				for (const dep of migration.dependencies) {
					if (
						!this.appliedMigrations.has(dep) &&
						!toApply.find((m) => m.version === dep)
					) {
						conflicts.push(
							`Migration ${migration.version} depends on ${dep} which is not applied`
						);
					}
				}
			}
	...
	async migrate(targetVersion?: string): Promise<void> {
		...
		if (plan.conflicts.length > 0) {
			throw new MigrationError(
				`Migration conflicts: ${plan.conflicts.join(', ')}`
			);
		}

		this.logger.info(
			{ module: 'MigrationManager', method: 'migrate' },
	...
	async rollback(targetVersion?: string): Promise<void> {
		const migrationsToRollback =
			this.getMigrationsToRollback(targetVersion);

		this.logger.info(
			{ module: 'MigrationManager', method: 'rollback' },
	...
	createMigration(name: string, description?: string): Migration {
		...
		const migration: Migration = {
			...
			down: async () => {
				throw new MigrationError(
					'Migration down method not implemented'
				);
			},
			rollbackable: true,
		};
