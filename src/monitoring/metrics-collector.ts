...
export class MetricsCollector extends EventEmitter {
  ...
  private thresholds = new Map<string, { value: number; operator: '>' | '<' | '=' }>();
   ...
   * Увеличение счетчика
   */
  increment(name: string, increment = 1, tags?: Record<string, string>): void {
    this.record({
      name,
      type: 'counter',
      value: increment,
   ...
   * Запись гистограммы
   */
  histogram(name: string, value: number, buckets: Record<string, number>, tags?: Record<string, string>): void {
    this.record({
      name,
      type: 'histogram',
      value,
   ...
   */
  setThreshold(name: string, value: number, operator: '>' | '<' | '=' = '>'): void {
  ...
  private checkThresholds(metric: Metric): void {
    ...
    if (triggered) {
      ...
      this.logger.warn({ module: 'MetricsCollector', method: 'checkThresholds' }, 
   ...
   */
  getAggregatedMetrics(name: string, period = '1h'): AggregatedMetrics | null {
    const now = new Date();
  ...
  private cleanupOldMetrics(): void {
    ...
    if (removedCount > 0) {
      this.logger.debug({ module: 'MetricsCollector', method: 'cleanupOldMetrics' }, 
  ...
  getStats() {
    ...
    for (const metric of this.metrics) {
      metricTypes.set(metric.type, (metricTypes.get(metric.type) || 0) + 1);
    }
