...
export class QueryAnalyzer {
  ...
  getPerformanceStats() {
    ...
    const slowQueryRate = totalQueries > 0 ? slowQueryCount / totalQueries : 0;
    
    const avgComplexity = totalQueries > 0
      ? this.metrics.reduce((sum, m) => sum + m.complexity.score, 0) / totalQueries
      : 0;
...
export class QueryOptimizer {
   ...
   * Оптимизация индексов
   */
  private optimizeIndexes(query: string): { query: string; optimizations: Optimization[]; improvement: number } {
    const optimizations: Optimization[] = [];
    let improvement = 0;
   ...
   * Оптимизация JOIN'ов
   */
  private optimizeJoins(query: string): { query: string; optimizations: Optimization[]; improvement: number } {
    const optimizations: Optimization[] = [];
    let improvement = 0;

    // Поиск неэффективных JOIN'ов
    const joinMatches = query.match(/JOIN\s+(\w+)\s+ON\s+([^WHERE|ORDER|GROUP|LIMIT]+)/gi);
    if (joinMatches) {
   ...
   * Оптимизация фильтров
   */
  private optimizeFilters(query: string): { query: string; optimizations: Optimization[]; improvement: number } {
    const optimizations: Optimization[] = [];
    let improvement = 0;
   ...
   * Оптимизация проекции
   */
  private optimizeProjection(query: string): { query: string; optimizations: Optimization[]; improvement: number } {
    const optimizations: Optimization[] = [];
    let improvement = 0;
    ...
    if (query.includes('SELECT *')) {
      optimizations.push({
        type: 'projection',
        description: 'Consider selecting only required fields instead of *',
        impact: 'low'
      });
      improvement += 5;
    }
...
export class PerformanceManager {
  ...
  getOptimizationRecommendations(): string[] {
    ...
    if (stats.slowQueryRate > 0.1) {
      recommendations.push('High slow query rate detected. Consider optimizing frequently used queries.');
    }
    ...
    if (stats.avgComplexity > 20) {
      recommendations.push('High query complexity detected. Consider breaking down complex queries.');
    }
