...
import { QueryError, TimeoutError, RateLimitError } from '../errors/index.js';
...
export class QueryEngine {
  ...
  constructor(
    ...
    this.config = {
      ...
      rateLimit: config.rateLimit ?? { requestsPerSecond: 100, burstLimit: 10 }
    };
  ...
  async query<T = any>(
    ...
    for (let attempt = 0; attempt <= this.config.retryAttempts; attempt++) {
      try {
        ...
        if (this.config.enableCaching && this.cache && this.isCacheableQuery(sql)) {
          await this.cache.setQueryResult(queryHash, result, this.config.cacheTTL);
        }
        ...
        if (attempt < this.config.retryAttempts) {
          this.logger.warn({ module: 'QueryEngine', method: 'query' }, 
            `Query failed, retrying (${attempt + 1}/${this.config.retryAttempts})`, error);
   ...
   */
  private async executeQuery<T>(sql: string, params?: QueryParams): Promise<T> {
  ...
  async transaction<T>(
    ...
    try {
      ...
      try {
        ...
        this.logger.error({ module: 'QueryEngine', method: 'transaction' }, 
  ...
  private isCacheableQuery(sql: string): boolean {
    ...
    const nonCacheableKeywords = [
      ...
      'BEGIN', 'COMMIT', 'CANCEL', 'SIGNIN', 'SIGNUP'
    ];
  ...
  private logQuery(stats: QueryStats): void {
    if (this.config.enableQueryLogging) {
      this.logger.info({ module: 'QueryEngine', method: 'logQuery' }, 
        `Query executed: ${stats.duration}ms ${stats.cached ? '(cached)' : ''}`, {
          query: stats.query,
