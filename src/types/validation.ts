...
export class SchemaValidator {
	...
	validateSchema(): ValidationResult {
		...
		for (const [tableName, tableConfig] of Object.entries(this.schema)) {
			...
			const tableValidation = this.validateTableConfig(
				tableName,
				tableConfig
			);
	...
	private validateFieldValue(
		...
		if (config.constraints) {
			const constraintsValidation = this.validateConstraints(
				fieldName,
				sanitizedValue,
				config.constraints
			);
		...
		switch (expectedType) {
			...
			case 'array':
				if (!Array.isArray(value)) {
					// Попытка санитизации строки в массив
					if (typeof value === 'string') {
						// Разделение по запятым для простых случаев
						if (value.includes(',')) {
