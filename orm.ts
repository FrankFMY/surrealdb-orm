...
export interface FieldConfig {
	...
	reference?: {
		onDelete?: 'REJECT' | 'CASCADE' | 'IGNORE' | 'UNSET' | { then: string };
	};
	/** Комментарий к полю */
	comment?: string;
	...
	permissions?:
		| 'FULL'
		| 'NONE'
		| { select?: string; create?: string; update?: string };
}
...
export class Table<Config extends TableConfig> {
	 ...
	 */
	async createRecord(
		data: Omit<TableRecord<Config>, 'id' | 'zip' | 'created' | 'updated'>
	): Promise<TableRecord<Config>> {
		const id = `${this.tableName}:${Date.now()}`;
		const now = Date.now();
		const payload = { ...data, id, created: now, updated: now } as Record<
			string,
			unknown
		>;
	...
	async updateRecord(
		...
		const updateData = { ...data, updated: Date.now() } as Record<
			string,
			unknown
		>;
	...
	async upsertRecord(
		id: string,
		data: Partial<TableRecord<Config>>
	): Promise<TableRecord<Config>> {
		const now = Date.now();
	...
	private generateCreateTableSQL(): string {
		...
		if (this.config.permissions) {
			if (
				this.config.permissions === 'FULL' ||
				this.config.permissions === 'NONE'
			) {
				tableDef += ` PERMISSIONS ${this.config.permissions}`;
			} else {
				tableDef += ` PERMISSIONS`;
				if (this.config.permissions.select)
					tableDef += ` FOR select ${this.config.permissions.select}`;
				if (this.config.permissions.create)
					tableDef += ` FOR create ${this.config.permissions.create}`;
				if (this.config.permissions.update)
					tableDef += ` FOR update ${this.config.permissions.update}`;
				if (this.config.permissions.delete)
					tableDef += ` FOR delete ${this.config.permissions.delete}`;
			}
		}
		tableDef += `;`;
		parts.push(tableDef);
	...
	async planDiff(): Promise<string> {
		...
		const parts: string[] = [];
		// Поля
		for (const [fieldName, fieldCfg] of Object.entries(
			this.config.fields
		)) {
			...
			if (!have) {
				...
				const want = this.generateFieldSQL(fieldName, fieldCfg).replace(
					'IF NOT EXISTS ',
					'OVERWRITE '
				);
		...
		if (this.config.indexes) {
			for (const idx of this.config.indexes) {
				...
				if (!current) parts.push(this.generateIndexSQL(idx));
					...
					const want = this.generateIndexSQL(idx).replace(
						'IF NOT EXISTS ',
						'OVERWRITE '
					);
		...
		if (this.config.constraints) {
			for (const c of this.config.constraints) {
				...
				const want = this.generateConstraintSQL(c).replace(
					'IF NOT EXISTS ',
					'OVERWRITE '
				);
		...
		if (this.config.triggers) {
			for (const t of this.config.triggers) {
				...
				const want = this.generateTriggerSQL(t).replace(
					'IF NOT EXISTS ',
					'OVERWRITE '
				);
		...
		const alters: string[] = [];
		const currentTable = (info as any).table as
			| {
					schema?: string;
					permissions?: TableConfig['permissions'];
					comment?: string;
			...
			| undefined;
		const samePerms = (
			a?: TableConfig['permissions'],
			b?: TableConfig['permissions']
		) => {
			if (!a && !b) return true;
			if (!a || !b) return false;
		...
		if (
			this.config.schema &&
			this.config.schema !== (currentTable?.schema as any)
		) {
		...
		if (
			...
			if (
				...
				this.config.permissions &&
				typeof this.config.permissions === 'object'
			) {
				const p: string[] = [];
				if (this.config.permissions.select)
					p.push(`FOR select ${this.config.permissions.select}`);
				if (this.config.permissions.create)
					p.push(`FOR create ${this.config.permissions.create}`);
				if (this.config.permissions.update)
					p.push(`FOR update ${this.config.permissions.update}`);
				if (this.config.permissions.delete)
					p.push(`FOR delete ${this.config.permissions.delete}`);
				if (p.length) alters.push(`PERMISSIONS ${p.join(' ')}`);
			}
		}
		if (
			this.config.comment !== undefined &&
			this.config.comment !== (currentTable?.comment ?? undefined)
		) {
			alters.push(`COMMENT ${JSON.stringify(this.config.comment)}`);
		}
		if (alters.length)
			parts.push(`ALTER TABLE ${this.tableName} ${alters.join(' ')};`);
		return parts.join('\n');
	}
	...
	private generateFieldSQL(fieldName: string, config: FieldConfig): string {
		...
		if (
			...
			if (config.type === 'record' && config.references)
				fieldDef += ` TYPE record<${config.references}>`;
			else if (config.type === 'array' && config.arrayOf) {
				if (typeof config.arrayOf === 'string')
					fieldDef += ` TYPE array<${config.arrayOf}>`;
				else if ((config.arrayOf as any).record)
					fieldDef += ` TYPE array<record<${
						(config.arrayOf as any).record
					}>>`;
				else if (
					typeof config.arrayOf === 'object' &&
					(config.arrayOf as any).type === 'record' &&
					(config.arrayOf as any).references
				)
					fieldDef += ` TYPE array<record<${
						(config.arrayOf as any).references
					}>>`;
					...
					typeof config.arrayOf === 'object' &&
					(config.arrayOf as any).object
				)
					fieldDef += ` TYPE array<object>`;
		...
		if (
			(config.type === 'record' && config.references) ||
			(config.type === 'array' && (config.arrayOf as any)?.record)
		) {
			const del = config.reference?.onDelete;
			if (del) {
				if (typeof del === 'string')
					fieldDef += ` REFERENCE ON DELETE ${del}`;
				else if (typeof del === 'object' && del.then)
					fieldDef += ` REFERENCE ON DELETE THEN ${del.then}`;
			}
		}
		...
		if (config.permissions) {
			if (
				config.permissions === 'FULL' ||
				config.permissions === 'NONE'
			) {
				fieldDef += ` PERMISSIONS ${config.permissions}`;
			} else {
				fieldDef += ` PERMISSIONS`;
				if (config.permissions.select)
					fieldDef += ` FOR select ${config.permissions.select}`;
				if (config.permissions.create)
					fieldDef += ` FOR create ${config.permissions.create}`;
				if (config.permissions.update)
					fieldDef += ` FOR update ${config.permissions.update}`;
			}
		}
		...
		if (config.constraints) {
			for (const [constraint, value] of Object.entries(
				...
				if (
					constraint === 'string::len' &&
					Array.isArray(value) &&
					value.length === 2
				) {
					const [min, max] = value as [number, number];
					fieldDef += ` ASSERT string::len($value) >= ${JSON.stringify(
						min
					)} AND string::len($value) <= ${JSON.stringify(max)}`;
					continue;
				}
				...
				if (Array.isArray(value)) {
					fieldDef += ` ASSERT ${constraint}($value, [${value
						.map((v) => JSON.stringify(v))
						.join(', ')}])`;
				} else if (typeof value === 'boolean') {
					fieldDef += ` ASSERT ${constraint}($value)`;
				} else {
					fieldDef += ` ASSERT ${constraint}($value, ${JSON.stringify(
						value
					)})`;
				}
			}
		...
		if (config.type === 'object' && config.properties) {
			for (const [child, childCfg] of Object.entries(config.properties)) {
				parts.push(
					this.generateFieldSQL(`${fieldName}.${child}`, childCfg)
				);
			}
		}
		...
		if (
			...
			const obj = (config.arrayOf as any).object as {
				properties?: Record<string, FieldConfig>;
			};
			for (const [child, childCfg] of Object.entries(
				obj.properties || {}
			)) {
				parts.push(
					this.generateFieldSQL(`${fieldName}.*.${child}`, childCfg)
	...
	private generateIndexSQL(index: {
		...
		if (index.search) {
			const parts: string[] = [];
			parts.push(
				`DEFINE INDEX IF NOT EXISTS ${index.name} ON TABLE ${
					this.tableName
			...
			if (
				index.search.bm25 &&
				(index.search.bm25.k1 !== undefined ||
					index.search.bm25.b !== undefined)
			) {
		...
		return `DEFINE INDEX IF NOT EXISTS ${index.name} ON TABLE ${
			this.tableName
		} FIELDS ${index.fields.join(', ')}${unique};`;
	}

	/**
	 * Генерация SQL для ограничения
	 */
	private generateConstraintSQL(constraint: {
		name: string;
		expression: string;
	}): string {
	 ...
	 */
	private generateTriggerSQL(trigger: {
		name: string;
		event: string;
		expression: string;
	}): string {
...
export class TableQuery<RecordType> {
	...
	toSQL(): { sql: string; vars: Record<string, unknown> } {
		...
		else if (this._withIndexes?.indexes?.length)
			sql += ` WITH INDEX ${this._withIndexes.indexes.join(', ')}`;
		if (this._version) sql += ` VERSION ${this._version}`;
		if (this._split) sql += ` SPLIT ${this._split}`;
		if (this._where) sql += ` WHERE ${this._where}`;
		const rangeParts: string[] = [];
		if (this._idRange?.from)
			rangeParts.push(
				`id >= type::thing($table, ${JSON.stringify(
					this._idRange.from
				)})`
			);
		if (this._idRange?.to)
			rangeParts.push(
				`id <= type::thing($table, ${JSON.stringify(this._idRange.to)})`
			);
		if (this._cursorAfter)
			rangeParts.push(`id > ${JSON.stringify(this._cursorAfter)}`);
		if (this._cursorBefore)
			rangeParts.push(`id < ${JSON.stringify(this._cursorBefore)}`);
		if (rangeParts.length)
			sql +=
				(this._where ? ` AND ` : ` WHERE `) + rangeParts.join(' AND ');
...
export class SurrealORM<Schema extends DatabaseSchema> {
	...
	constructor(rpc: SurrealRPC<TypedDatabaseSchema<Schema>>, schema: Schema) {
		...
		for (const [tableName, config] of Object.entries(schema)) {
			this.tables.set(
				tableName,
				new Table(rpc, tableName, config) as Table<Schema[keyof Schema]>
			);
		}
	}
	...
	private parseFieldType(
		typeStr?: string
	): Pick<FieldConfig, 'type' | 'references' | 'arrayOf'> {
	...
	async introspectTable(table: string): Promise<TableConfig> {
		...
		if (Array.isArray(info.fields)) {
			for (const f of info.fields) {
				...
				if (f.value_expr || f.value)
					cfg.valueExpr = f.value_expr ?? f.value;
				if (f.default_always === true) cfg.defaultAlways = true;
				if (f.reference?.onDelete)
					cfg.reference = { onDelete: f.reference.onDelete };
				fields[name] = cfg;
			}
			...
			for (const [name, def] of Object.entries<any>(info.fields)) {
				...
				const mDef = defStr.match(
					/\bDEFAULT(\s+ALWAYS)?\s+([^;]+?)(?:\s+READONLY|\s+ASSERT|\s+PERMISSIONS|;)/i
				);
				if (mDef) {
					...
					if (!/[()\w]+::/.test(expr)) {
						try {
							...
							if (/^".*"$/.test(expr) || /^'.*'$/.test(expr))
								cfg.default = expr.replace(/^['"]|['"]$/g, '');
						}
					}
				}
				// VALUE expr
				const mVal = defStr.match(
					/\bVALUE\s+([^;]+?)(?:\s+READONLY|\s+ASSERT|\s+PERMISSIONS|;)/i
				);
				if (mVal) cfg.valueExpr = mVal[1].trim();
				// READONLY
				if (/\bREADONLY\b/i.test(defStr)) cfg.readonly = true;
				// REFERENCE ON DELETE ...
				const mRef = defStr.match(
					/REFERENCE\s+ON\s+DELETE\s+(REJECT|CASCADE|IGNORE|UNSET|THEN\s+[^\s;]+)/i
				);
				if (mRef) {
					const v = mRef[1];
					cfg.reference =
		...
		const ensureObject = (
			parent: Record<string, FieldConfig>,
			key: string
		): FieldConfig => {
			if (!parent[key])
				parent[key] = { type: 'object', properties: {} } as any;
			if (parent[key].type !== 'object')
				parent[key] = { type: 'object', properties: {} } as any;
			return parent[key];
		};
		const ensureArrayObject = (
			parent: Record<string, FieldConfig>,
			key: string
		): FieldConfig => {
			if (!parent[key])
				parent[key] = {
					type: 'array',
					arrayOf: { object: { properties: {} } },
				} as any;
			if (
				parent[key].type !== 'array' ||
				!(parent[key] as any).arrayOf ||
				!(parent[key] as any).arrayOf.object
			) {
				parent[key] = {
					type: 'array',
					arrayOf: { object: { properties: {} } },
		...
		const addNested = (fullName: string, cfg: FieldConfig) => {
			...
			for (let i = 0; i < tokens.length; i++) {
				...
				if (nextTok === '*') {
					...
					const arrObj = (currentContainer as any).arrayOf
						.object as any as {
						properties?: Record<string, FieldConfig>;
					};
				...
				if (isLast) {
					if (currentContainer) {
						if (currentContainer.type === 'object') {
							...
							currentContainer.type === 'array' &&
							(currentContainer as any).arrayOf?.object
						) {
							const arrObj = (currentContainer as any).arrayOf
								.object as {
								properties?: Record<string, FieldConfig>;
		...
		if (info.indexes && typeof info.indexes === 'object') {
			for (const [idxName, idx] of Object.entries<any>(info.indexes)) {
				...
				let search:
					| NonNullable<TableConfig['indexes']>[number]['search']
					| undefined;
				...
				if (typeof sql === 'string') {
					...
					const mSearch = sql.match(
						/SEARCH\s+ANALYZER\s+([\w:.-]+)/i
					);
					if (mSearch) {
						...
						if (/\bHIGHLIGHTS\b/i.test(sql))
							(search as any).highlights = true;
					}
				}
				indexes!.push({
					name: idxName,
	...
	async relate(
		...
		const sql = `RELATE type::thing($from_tb, $from_id) -> $edge -> type::thing($to_tb, $to_id) ${
			data ? 'CONTENT $data' : ''
		} RETURN AFTER`;
	...
	async live(
		...
		let unsubscribe = () => {};
		if (options?.onEvent)
			unsubscribe = (this.rpc as any).subscribeLive(
				queryId,
				options.onEvent
			);
		return {
			id: queryId,
			unsubscribe,
			kill: () => (this.rpc as any).kill(queryId),
		};
	}
}
 ...
 */
export function createORM<Schema extends DatabaseSchema>(
	rpc: SurrealRPC<any>,
	schema: Schema
): SurrealORM<Schema> {
