...
function analyzeStatement(stmt: string): DetailedItem {
	...
	const action: DetailedItem['action'] =
		up.includes(' OVERWRITE ') ? 'overwrite'
		: up.startsWith('ALTER TABLE') ? 'alter'
		: up.includes(' IF NOT EXISTS ') ? 'create'
		: 'define';
	...
	m = s.match(
		/^DEFINE\s+FIELD\s+(?:OVERWRITE|IF NOT EXISTS\s+)?(\w+)\s+ON\s+(?:TABLE\s+)?(\w+)\s+(.+);?$/i
	);
	if (m) {
		const rest = m[3];
		...
		if (/\bPERMISSIONS\b/i.test(rest)) aspects.push('permissions');
		if (/REFERENCE\s+ON\s+DELETE/i.test(rest))
			aspects.push('reference.onDelete');
		return {
			kind: 'field',
			table: m[2],
			name: m[1],
	...
	m = s.match(
		/^DEFINE\s+INDEX\s+(?:OVERWRITE|IF NOT EXISTS\s+)?(\w+)\s+ON\s+(?:TABLE\s+)?(\w+)\s+(.+);?$/i
	);
	if (m) {
		const rest = m[3];
	...
	m = s.match(
		/^DEFINE\s+EVENT\s+(?:OVERWRITE|IF NOT EXISTS\s+)?(\w+)\s+ON\s+(?:TABLE\s+)?(\w+)\s+/i
	);
	if (m) {
...
function analyze(sql: string) {
	...
	const groups: Record<string, DetailedItem[]> = {
		tables: [],
		fields: [],
		indexes: [],
		events: [],
...
async function planDestructive(rpc: any, schema: any) {
	...
	const liveTables =
		...
		: db.tables ? Object.keys(db.tables)
		: [];
	const schemaTables = Object.keys(schema || {});
	// tables to drop
	for (const t of liveTables) {
		if (!schemaTables.includes(t))
			droppables.push({ kind: 'table', table: t, action: 'drop' });
	}
	...
	for (const t of liveTables) {
		...
		const sIndexes = new Set(
			(sTable.indexes || []).map((i: any) => i.name)
		);
		const sEvents = new Set([
			...(sTable.constraints || []).map((c: any) => c.name),
			...(sTable.triggers || []).map((c: any) => c.name),
		]);
		// fields
		for (const fname of Object.keys(info.fields || {})) {
			if (!sFields.includes(fname))
				droppables.push({
					kind: 'field',
					table: t,
					name: fname,
...
async function main() {
	...
	const schema =
		schemaPath ? JSON.parse(readFileSync(schemaPath, 'utf8')) : {};
	...
	if (action === 'plan') {
		...
		const analyzed =
			sql.trim() ?
				analyze(sql)
			:	{ total: 0, items: [], groups: {} as Record<string, unknown[]> };
		let drops: DetailedItem[] = [];
		if (destructive) drops = await planDestructive(rpc, schema);
		const groups = (analyzed.groups as Record<string, unknown[]>) || {};
		const summary = {
			total: analyzed.total || 0,
			tables: groups.tables?.length || 0,
			fields: groups.fields?.length || 0,
			indexes: groups.indexes?.length || 0,
			events: groups.events?.length || 0,
			alters: groups.alters?.length || 0,
		...
		if (json) {
			console.log(
				JSON.stringify(
					{ ...analyzed, destructive: drops, summary },
					null,
					2
				)
			);
			...
			for (const [group, arr] of Object.entries(groups)) {
				...
				for (const it of arr) {
					...
					const head = [item.table, item.name]
						.filter(Boolean)
						.join('.');
					const aspects =
						item.aspects?.length ?
						...
						:	'';
					console.log(
						`- ${item.action} ${item.kind}${head ? ' ' + head : ''}${aspects}`
					);
			...
			if (destructive) {
				...
				for (const d of drops) {
					...
					const aspects =
						d.aspects?.length ? ` [${d.aspects.join(', ')}]` : '';
	...
	console.error(
		'Unknown action. Use: plan | apply [--json] [--detail] [--schema schema.json] [--destructive]'
	);
	process.exit(2);
}
